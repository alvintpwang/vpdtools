...
parser = argparse.ArgumentParser(description='Reverses a Single Record VPD image into XML template files', add_help=False, formatter_class=argparse.RawDescriptionHelpFormatter,
                                 epilog=textwrap.dedent('''\
                                 Examples:
                                   ./reverseSingleRecordVpd.py -v image.vpd -o /tmp
                                 '''))
# Create our group of required command line args
reqgroup = parser.add_argument_group('Required Arguments')
reqgroup.add_argument('-v', '--vpdfile', help='The valid single record vpd formatted input file', required=True)
...
# Create separate tvpd
clCreateRecords = true
...
# There is only single record
# Get the record name
offset = 6
recordName = vpdContents[offset:(offset + 4)].decode()
# Create the entry with the name
recordNames[recordName] = RecordInfo()
recordNames[recordName].recordName = recordName
# recordOffset
recordNames[recordName].recordOffset = 0
